...
We have seen previously that the suitable number of PCA components to use was four, since they are able to explain the 89% of the variability of the data. In order to apply factor analysis to the data, we will use again only quantitative variables scaled. Nevertheless, in this case we are going to include the variable _LSTAT_.

Factor analysis allow us to identify if there is an association between some observable quantitative attributes and some latent unobservable variables, .i.e. the **factors**. We have to estimate matrix M and use _varimax rotation_ in order to interpret the relationship between variables and factors. The number of factors, as we have commented, will be four. Then, _varimax rotation_ is applied and these four factors we have selected will be analyzed.

```{r}
X_factors = dataset.with.categories  %>% mutate(
  CHAS = NULL,
  RAD.CAT = NULL,
  RM.CAT = NULL,
  INDUS.CAT = NULL,
...
pcfa_matrix <- Y_pcs$rotation[, 1:r] %*% diag(Y_pcs$sdev[1:r])
pcfa_matrix <- loadings(varimax(pcfa_matrix))[1:p, 1:r]
fviz_eig(Y_pcs, ncp=p, addlabels=T, barfill=colors[1], barcolor=colors[4])
knitr::kable(get_eigenvalue(Y_pcs), digits=4) %>% 
  column_spec(1, bold=TRUE) %>% 
  row_spec(0, bold=TRUE) %>% 
  kable_styling(latex_options = "HOLD_position")
```
Once matrix M has been computed, we analyze the importance of variables for each factor vector obtained. We can do it interpreting the following plots, let's take a look to the plot associated to the first principal component factor analysis:
...
text(1:p,
     pcfa_matrix[,1],
     labels=colnames(X_factors),
     pos=1,
     col=colors[3],
     cex=0.8)
```
The variables *INDUS*, *NOX*, *AGE*, *ZN* and *DIS* represents the most important feature to define the first latent variable. In this case, the first three variables are opposite to the second last ones. We could see that this first factor defines and index for the type of the area (residential vs business places).

Let's take a look to the second principal component factor analysis:

```{r, fig.height=7}
plot(1:p,pcfa_matrix[,2],
     pch=19, 
     col=colors[1],
     xlab="",
     ylab="Weights",
     main="Weights for the second factor")
abline(h=0)
text(1:p,pcfa_matrix[,2], 
     labels=colnames(X_factors), 
     pos=1, 
     col=colors[3], 
     cex=0.8)
```
The second factor are defined by the variables *RM* and *MEDV* on the top and on the bottom the variable *LSTAT*. These attributes characterize this second latent variable, showing an index for the class of the area. On one hand, higher values for the number of rooms and for the median price of the owner-occupied homes are associated with areas with large purchasing power. On the other hand, high percentage of lower status of the population is associated with poor neighborhoods. 

Let's take a look to the third principal component factor analysis:

```{r, fig.height=7}
plot(1:p,pcfa_matrix[,3],
     pch=19, 
     col=colors[1],
     xlab="",
     ylab="Weights",
     main="Weights for the third factor")
abline(h=0)
text(1:p,pcfa_matrix[,3], 
     labels=colnames(X_factors), 
     pos=1, 
     col=colors[3], 
     cex=0.8)
...
In this case, the variables with largest weight in magnitude are *CRIM*, *RAD*, *TAX* and *B*. The first three high variables are high correlated, and they are opposite to *B*. This factor could be an index of criminality, nevertheless we do not want to talk to much about the variable *B* as we consider it kind racist.

Let's see what information provides the forth principal component factor analysis:

```{r, fig.height=7}
plot(1:p,pcfa_matrix[,4],
     pch=19, 
     col=colors[1],
     xlab="",
     ylab="Weights",
     main="Weights for the forth factor")
abline(h=0)
text(1:p,pcfa_matrix[,4], 
     labels=colnames(X_factors), 
     pos=1, 
     col=colors[3], 
     cex=0.8)
...
This forth factor provides the unobservable information about that the big residential areas (*ZN*) has a lower ratio of pupil-teacher (*PTRATIO*) compared to the places out of there, therefore this factor appears to be an index of schooling rates for each area.
...
text(1:p,sort(comm_pcfa,decreasing=TRUE),
     labels=names(sort(comm_pcfa,decreasing=TRUE)),
     pos=4,
     col=colors[5],
     cex=0.75)
...
The variables better explained by the factors are *RAD*, *PTRATIO*, *DIS*, *TAX* while variables worst explained by the factors are *CRIM* and *B*.

```{r, fig.height=7}
uniq_pcfa <- 1 - comm_pcfa
names(uniq_pcfa) <- names(comm_pcfa)

plot(1:p,sort(uniq_pcfa,decreasing=TRUE),
     pch=20,
     col=colors[1],
     xlim=c(0,34),
     xlab="Variables",
...
text(1:p,sort(uniq_pcfa,decreasing=TRUE),
     labels=names(sort(uniq_pcfa,decreasing=TRUE)),
     pos=4,
     col=colors[5],
     cex=0.75)
```
The uniqueness plot is the same but opposite, the largest variable of the communality is the smallest in the uniqueness and vice versa.

Therefore, the factors provide a measure of the type of the area (residential or business), the purchase power of the neighborhood, the criminality and the schooling rate. 

The next step is to analyze the score factors.

```{r}
Sigma_nu_pcfa <- diag(diag(cov(Y) - pcfa_matrix %*% t(pcfa_matrix)))

F_pcfa <- Y %*% solve(Sigma_nu_pcfa) %*% pcfa_matrix %*% 
  solve(t(pcfa_matrix) %*% solve(Sigma_nu_pcfa) %*% pcfa_matrix)
colnames(F_pcfa) <- c("Factor 1","Factor 2","Factor 3","Factor 4")
pairs(F_pcfa, pch=19, col=colors[1])
corrplot(cor(F_pcfa), order="hclust")
```

In the tow plots above we can see that there is not a correlation between any pair of factors at all. This is something we have expected as each pair of principal component factor analysis are uncorrelated.

## Principal factor analysis

Obtain the sample correlation matrix of X, that is the sample covariance matrix of Y. Later on, let's get the R_X - Sigma_nu_pcfa and its eigenvectors and eigenvalues:
...
Let's estimate the matrix M and use the _varimax rotation_ for interpretability:

```{r}
M_pfa <- MM_eig$vectors[,1:r] %*% diag(MM_eig$values[1:r])^(1/2)
M_pfa <- varimax(M_pfa)
M_pfa <- loadings(M_pfa)[1:p,1:r]
colnames(M_pfa) <- c("First factor", "Second factor", 
                     "Third factor", "Forth factor")
rownames(M_pfa) <- names(X_factors)
knitr::kable(M_pfa) %>% 
  column_spec(1, bold=TRUE) %>% 
  row_spec(0, bold=TRUE) %>% 
  kable_styling(latex_options = "HOLD_position")
...
Let's take a look to the differences between the method we have used in the previous point, called _Principal Component Factor Analysis_, and this one:
...
plot(pcfa_matrix[,1],
     ...
     pch=19,
     col=colors[1],
     main="First factors with PCFA and PFA",
     xlab="PCFA",
     ylab="PFA")
...
plot(pcfa_matrix[,2],
     ...
     pch=19,
     col=colors[1],
     main="Second factors with PCFA and PFA",
     xlab="PCFA",
     ylab="PFA")
plot(pcfa_matrix[,3],
     M_pfa[,3],
     pch=19,
     col=colors[1],
     main="Third factors with PCFA and PFA",
     xlab="PCFA",
     ylab="PFA")
plot(pcfa_matrix[,4],
     M_pfa[,4],
     pch=19,
     col=colors[1],
     main="Fourth factors with PCFA and PFA",
     xlab="PCFA",
     ylab="PFA")

```

We can realized that there is not a big difference between the previous procedure and this one. 

Let's plot the weights for the first factor:
...
This plot above shows that there is not noticeable difference. Let's plot the comparison of weights for the third factor of each method:
...
plot(1:p,
     ...
     xlab="",
     ylab="Weights",
     main=" PFA Weights for the third factor")
abline(h=0)
text(1:p,
     M_pfa[,3],
     labels=colnames(X),
...
plot(1:p,
     ...
     xlab="",
     ylab="Weights",
     main="PFCA Weights for the third factor")
abline(h=0)
text(1:p,
     pcfa_matrix[,3],
     labels=colnames(X)
...
The following is the covariance matrix of the errors:

```{r}
Sigma_nu_pfa <- diag(diag(R_X - M_pfa %*% t(M_pfa)))
colnames(Sigma_nu_pfa) <- names(X_factors)
rownames(Sigma_nu_pfa) <- names(X_factors)
knitr::kable(Sigma_nu_pfa, digits=3) %>% 
  column_spec(1, bold=TRUE) %>% 
  row_spec(0, bold=TRUE) %>% 
  kable_styling(latex_options = "HOLD_position")
```
...
Let's now compare the noise variance with _PCFA_ and _PFA_:
...
There are some small differences but not noticeable.

Let's show the communalities for each procedure used before:
...
comm_pfa_sort <- sort(comm_pfa, decreasing=TRUE)
comm_pcfa_sort <- sort(comm_pcfa, decreasing=TRUE)
comm <- rbind(comm_pfa_sort, comm_pcfa_sort)
rownames(comm) <- c("PFA", "PCFA")
knitr::kable(comm, digits=3, caption="Communalities") %>% 
  column_spec(1, bold=TRUE) %>% 
  row_spec(0, bold=TRUE) %>% 
  kable_styling(latex_options = "HOLD_position")
```
...
Let's show now the uniqueness for each procedure we have used:

```{r}
uniq_pfa <- diag(Sigma_nu_pfa)
names(uniq_pfa) <- names(comm_pfa)
uniq_pfa_sort <- sort(uniq_pfa,decreasing=TRUE)
uniq_pcfa_sort <- sort(uniq_pcfa,decreasing=TRUE)
uniq <- rbind(uniq_pfa_sort, uniq_pcfa_sort)
rownames(uniq) <- c("PFA", "PCFA")
knitr::kable(uniq, digits=3, caption="Uniqueness") %>% 
  column_spec(1, bold=TRUE) %>% 
  row_spec(0, bold=TRUE) %>% 
  kable_styling(latex_options = "HOLD_position")
```
...
The variables better explained by the factors are *RAD* and *PTRATIO*, whereas the worse ones are *B* and *CRIM*. These results are align with the conclusions that we have been obtaining through all this project.

Let's estimate now the factors:

```{r}
F_pfa <- Y %*% solve(Sigma_nu_pfa) %*% M_pfa %*% 
  solve(t(M_pfa) %*% solve(Sigma_nu_pfa) %*% M_pfa)
colnames(F_pfa) <- c("Factor 1","Factor 2","Factor 3","Factor 4")
head(F_pfa)
```

Let's see that the factors are uncorrelated:

```{r}
pairs(F_pfa,pch=19, col=colors[1])
corrplot(cor(F_pfa), order="hclust")
```
In the scatter plot we can see that now we are obtaining a perfect separation of at least two groups when the third component is added, whereas when we applied PCA the differenciation was't that clear. Also we can see that the factors are not correlated at all.

Let's obtain the correlation matrix between the PCFA and PFA estimates:
...
The previous methods of estimation don't required knowledge of the underlying distribution of $\mathbf{X}$. In the case, Maximum Likelihood Estimation method needs to work under Gaussian data, this means that the variables and the noise are normally distributed. If this is not assure, the procedure will not work properly and results will not succeed.  

First, we are going to use the _factanal_ method from _stats_ library to perform the _MLE_ factor analysis on the data matrix. Iterating from 1 factor to 7 factors and applying the LRT in order to see if the _r_ number of factors of the null hypothesis is statistically significant:
...
Y_mle_6 <- factanal(Y, factors=6, rotation="varimax", scores="Bartlett")
Y_mle_7 <- factanal(Y, factors=7, rotation="varimax", scores="Bartlett")
Y_mle_8 <- factanal(Y, factors=8, rotation="varimax", scores="Bartlett")

lrt.stat <- c(Y_mle_1$STATISTIC, Y_mle_2$STATISTIC, Y_mle_3$STATISTIC, 
              Y_mle_4$STATISTIC, Y_mle_5$STATISTIC, Y_mle_6$STATISTIC,
              Y_mle_7$STATISTIC, Y_mle_8$STATISTIC)
chisq.pvalue <- c(Y_mle_1$PVAL, Y_mle_2$PVAL, Y_mle_3$PVAL, Y_mle_4$PVAL,
                  Y_mle_5$PVAL, Y_mle_6$PVAL, Y_mle_7$PVAL, Y_mle_8$PVAL)
lrt.matrix <- cbind(1:8, lrt.stat, chisq.pvalue)
rownames(lrt.matrix) <- lrt.matrix[, 1]
colnames(lrt.matrix) <- c("r", expression(lambda), "p-value")

knitr::kable(lrt.matrix) %>%
  column_spec(1, bold=TRUE) %>% 
  row_spec(0, bold=TRUE) %>% 
  kable_styling(latex_options = "HOLD_position")
...
The null hypothesis is always rejected, then we choose _r_ equal to 4 factors as we have done in the previous procedures. As it occurred in the case of study at class, the test rejects small values for _r_ for non-Gaussian datasets. If we take a look to the following histogram plots, we can see that there are not so many variable with normal distribution (a few of them has discrete distribution):

```{r}
par(mfrow=c(3,5))
sapply(names(X_factors),function(cname){hist(X_factors[[cname]], main=cname, col=colors[2])})
```


Therefore, as it was suggested by the _LR_ test (although it is not valid for this case), we are going to choose 4 factors. First we are going to take the loading matrix:
...
The two estimates of the loading matrix are similar so we do not plot the estimated weights again. Nevertheless, we can see differences for the second and third factors.
...
comm_mle_sort <- sort(comm_mle,decreasing=TRUE)
comm_pfa_sort <-sort(comm_pfa,decreasing=TRUE)

comm <- rbind(comm_mle_sort, comm_pfa_sort)
rownames(comm) <- c("MLE", "PFA")
knitr::kable(comm, digits=3, caption="Communalities") %>% 
  column_spec(1, bold=TRUE) %>% 
  row_spec(0, bold=TRUE) %>% 
  kable_styling(latex_options = "HOLD_position")
...
uniq_mle_sort <- sort(uniq_mle,decreasing=TRUE)
uniq_pfa_sort <- sort(uniq_pfa,decreasing=TRUE)
uniq <- rbind(uniq_mle_sort, uniq_pfa_sort)
rownames(uniq) <- c("MLE", "PFA")
knitr::kable(uniq, digits=3, caption="Uniqueness") %>% 
  column_spec(1, bold=TRUE) %>% 
  row_spec(0, bold=TRUE) %>% 
  kable_styling(latex_options = "HOLD_position")
...
The variables better explained by the factors are *TAX*, *RAD* and *DIS*
The variables worst explained by the factors are *B*, *CRIM* and *LSTAT*
