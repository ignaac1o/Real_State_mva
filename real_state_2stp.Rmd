...
```
```{r}
kmeans_X <- kmeans(X,centers=3,iter.max=1000,nstart=100)
kmeans_X$cluster
```

```{r}
colors_kmeans_X <- c("green","blue","black")[kmeans_X$cluster]
...
plot(sil_kmeans_X,col="green")
```
### PAM
```{r}
pam_X <- pam(X,k=3,metric="manhattan",stand=FALSE)
```

# Make a plot of the first two PCs with the solution

```{r}
colors_pam_X <- c("green","blue","black")[pam_X$cluster]
...
```{r}
clara_X <- clara(X,k=3,metric="manhattan",stand=FALSE)
```
```{r}
colors_clara_X <- c("green","blue","black")[clara_X$cluster]
