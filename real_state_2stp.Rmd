...
dataset.cut$LSTAT %>%  factor(levels = c("Low-class","Medium-High-class"),labels = c(1,0))
...
```{r}
clean_data$LSTAT.CAT <- cut(clean_data$LSTAT, 
    breaks = c(-Inf, 25, Inf), 
    labels = c('Medium-High-class', 'Low-class'),
    right = FALSE)
...
#X_trans[,12] <- log(X[,12]+10)
#colnames(X_trans)[12] <- "log_LSTAT"
X_trans[,12] <- log(X[,12]+10)
colnames(X_trans)[12] <- "log_MEDV"
...
```{r}
plot(x=X_pcs$rotation[,1],y=X_pcs$rotation[,3],pch=19,col="green",main="Weights for the first four PCs")
abline(h=0,v=0)
text(x=X_pcs$rotation[,1],y=X_pcs$rotation[,3],labels=colnames(X),pos=1,col="blue",cex=0.75)
library(plotrix)
...
```{r}
colors_X <- c("black","blue")[1*(clean_data$LSTAT.CAT=="Medium-High-class")+1]
par(mfrow=c(1,1))
plot(X_pcs$x[,c(1,3)],pch=19,col=colors_X)
```
...
S <- S_shrink[1:p,1:p]
eig_S <- eigs_sym(S,12)
eigen_vectors_S <- eig_S$vectors
Z <- X_centred %*% eigen_vectors_S
dim(Z)
```


Now lets see if we can see something using first 4 pcas

```{r}
plot(Z[,6],pch=19,col="blue")
```
We could differenciate 2 groups.

Let's look at the eigen values
...
Obtain the proportion of variability explained by the PCs and the accumulated proportion of variability
Note that a large number of PCs is needed to explain a large variability

```{r}

prop_var <- eigen_values_S/sum(eigen_values_S)
acum_prop_var <- cumsum(eigen_values_S)/sum(eigen_values_S)
cbind(prop_var,acum_prop_var)[1:12,]
```


```{r}
par(mfrow=c(1,2))
plot(1:12,eigen_values_S[1:12],col="blue",type="b",xlab="Number",ylab="Eigenvalues",pch=19,main="Eigenvalues of shrinkage covariance")
plot(1:12,acum_prop_var[1:12],col="blue",type="b",xlab="Number",ylab="Eigenvalues",pch=19,main="Accumulated eigenvalues")

```
Apparently, the first three PCs are the ones that explain most of the data set
...
par(mfrow=c(1,1))
plot(1:p,eigen_vectors_S[,1],pch=20,col="blue",main="Loadings for the first PC",
     xlab="Gene",ylab="First PC score")
abline(h=0,lwd=2)
```

```{r}
plot(1:p,eigen_vectors_S[,2],pch=20,col=color_1,main="Loadings for the second PC",
     xlab="Gene",ylab="Second PC score")
abline(h=0,lwd=2)
```

Again we can see two columns with very low values.


```{r}
plot(eigen_vectors_S[,1:2],pch=20,col=color_1,main="Loadings for the first two PCs",
     xlab="First PC",ylab="Second PC")
abline(h=0,v=0)
library(plotrix)
draw.circle(0,0,0.03,border=color_4,lwd=3)
```

```{r}
pairs(Z[,1:9],col=color_1,pch=19,main="The first nine PCs")
```
Fist pca is key for differentiating groups.
