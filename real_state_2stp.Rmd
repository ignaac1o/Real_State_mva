...
```
This method is useful when the curve has an important decrease first and then h the value of *WSS_k* stabilizes. Therefore, the plot do not allow us identify clearly the optimal number for *k* as we cannot see that significant decrease mentioned. However, the most important decline correspond to *k=2*, although we apply the rest of methods to check this selection.
...
```
Even although there are instances overlapping, both of clusters are well differentiated. Let's see how well is the assignment done.
...
We also implement CLARA and we will see the results.


```{r}
clara_X <- clara(X,k=2,metric="manhattan",stand=FALSE)
colors_clara_X <- c("green","blue")[clara_X$cluster]
plot(X_pcs$x[,c(1,3)],pch=19,col=colors_clara_X,main="First and third PCs",xlab="First PC",ylab="Third PC")
```
...
The results are quite similar to the results obtained with PAM algorithm as the dataset is not so large.
...
```{r}
X.qual <- clean_data
X.qual$CHAS <- as.factor(X.qual$CHAS)
```
```{r}
