...
```{r}
colors_CHAS <- c("red","blue")[1*(clean_data$CHAS==0)+1]
colors_LSTAT <- c("orange","green")[1*(clean_data$LSTAT==0)+1]
par(mfrow=c(1,2))
plot(X_pcs$x[,c(1,3)],pch=19,col=colors_CHAS)
plot(X_pcs$x[,c(1,3)],pch=19,col=colors_LSTAT)
```

Analyzing both of this plot we can conclude by saying that neither the CHAS feature and LSTAT are nor directly correlated with this categorization. 
