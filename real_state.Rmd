...
The main characteristics of the dataset without outliers, generated by the _MCD_ procedure, are:

```{r}
knitr::kable(rbind(mean.mcd.estimators), caption="Mean vector without outliers")
knitr::kable(covariance.mcd.estimators, caption="Covariance matrix without outliers")
knitr::kable(correlation.mcd.estimators, caption="Correlation matrix without outliers")
```
...
plot(1:n,x.sq.mah.mcd,pch=19,col=var.outliers.mah.mcd,main="Squared Mahalanobis distances",xlab="Observation",ylab="Squared Mahalanobis distance")
abline(h=qchisq(.99,p),lwd=3,col=colors[1])
plot(1:n,log(x.sq.mah.mcd),pch=19,col=var.outliers.mah.mcd,main="Log of squared Mahalanobis distances",xlab="Observation",ylab="Log of squared Mahalanobis distance")
abline(h=log(qchisq(.99,p)),lwd=3,col=colors[1])
```

We can see the outliers in the dataset using two graph we have already used previously:

- PCP

```{r}
pairs(X,pch=19,col=var.outliers.mah.mcd)

parcoord(X,col=var.outliers.mah.mcd,var.label=TRUE,main="PCP for milk contents")
```
...
```{r}
out_X <- rep(1,n)
out_X[outliers.mah.mcd] <- 2
andrews(as.data.frame(cbind(clean_data,as.factor(out_X))),clr=9,ymax=4,main="Andrews' Plot for milk contents")
```
